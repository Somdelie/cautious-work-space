...
async function upsertProduct(name, shortcut) {
...
async function upsertSupplierProduct(supplierId, productId) {
...
// ---- seed: PDF price list
async function seedPriceListFromPdf(pdfPath) {
...
async function seedSpreadRatesFromXlsx(xlsxPath) {
...
async function main() {
  ...
  const pdfPath = path.resolve(process.cwd(), "Plascon & Dulux March 2025.pdf");
  const spreadPath = path.resolve(
    process.cwd(),
    "Marmoran & Terraco Spread rates.xlsx",
  );
